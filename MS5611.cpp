...
bool MS5611::begin(ms5611_osr_t osr)
{
	...
	ms5611.prepareConversion(MS5611_CMD_CONV_D2);
	data_ready = false;
	P_flag = false;
...
void MS5611::timer1Init(void) {
	...
	if ( F_CPU == 8000000 ) {
		OCR1A = ct * 1000;			// Output Compare Register in micro seconds, 8 MHz MCU
	} else {
		if ( F_CPU == 16000000 ) {
			OCR1A = ct * 2000;		// Output Compare Register in micro seconds * 2, 16 MHz MCU
		}
	}	
	TCCR1B |= (1 << WGM12);		// CTC mode
	TCCR1B |= (1 << CS11);    	// prescaler, 8 for 8 MHz MCU 
...
void MS5611::readRawTemperature(void)
{
	
    D2 = readRegister24(MS5611_CMD_ADC_READ);
}

void MS5611::readRawPressure(void)
{
	
    D1 = readRegister24(MS5611_CMD_ADC_READ);
}

void MS5611::prepareConversion(uint8_t cmd) {
...
// using D2
double MS5611::getTemperature(bool compensation)
{
...
ISR(TIMER1_COMPA_vect) {
	...
	if ( ms5611.P_flag ) {
		...
		ms5611.data_ready = true;						// to be used in loop()
		ms5611.prepareConversion(MS5611_CMD_CONV_D2);	// prepare for Temperature
		ms5611.P_flag = false;
		
	} else {
	
		ms5611.readRawTemperature();					// assigned to D2	
		ms5611.prepareConversion(MS5611_CMD_CONV_D1);	// prepare for Pressure
		ms5611.P_flag = true;
	}
}
